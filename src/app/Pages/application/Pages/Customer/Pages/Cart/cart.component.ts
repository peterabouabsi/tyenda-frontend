...
export class CartComponent implements OnInit{
  ...
  public cartResultConfig: CartConfig<CartStoreBasicView | CartItemBasicView> = {data: [], top: 10, skip: 0, loaded: false};

  constructor(private cartService: CartService) {
  }

  ngOnInit(): void {
    this.readData(this.cartResultConfig.top, this.cartResultConfig.skip);
  }

  /*--------- Read Stores/Items in your cart based on the activeIndex -------------*/
  public readData(top: number, skip: number){
    let type = "";
    if(this.activeFilterIndex == 1) type = "Stores"
    if(this.activeFilterIndex == 2) type = "Items"

    this.cartService.readCart(type, top, skip).subscribe((response: any) => {
      if(!response.error){
        this.cartResultConfig = {data: response, top: top, skip: skip, loaded: true};
      }
    });
  }
  /*--------- Read Stores/Items in your cart based on the activeIndex -------------*/
